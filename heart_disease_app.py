...
@st.cache_resource
def load_resources():
    model = joblib.load("final_rf_model.joblib")     # trained Random Forest 
    scaler = joblib.load("scaler_heart_disease.joblib")           # same scaler used on full X dataset
    return model, scaler
...
# -------------------------------------------------------------
st.markdown("---")
st.caption("Developed  by  Reddy | Tuned RF Model")
